...
// 2. Inline Type Object - We also define the datatype of the keys (TS only )- but not recommended
// 3. Using type aliases (TS only)
...
// class creation
class Person{
    ssn:string; // note semicolon here, not comma like all above
    firstName:string;
    lastName:string;

    // constructor keyword is used in TS, creates constructor method to assign data to class variables.
...
// only 4 th way we use in automation if needed, or 1st way